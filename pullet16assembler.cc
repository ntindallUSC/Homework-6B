...
void Assembler::PrintMachineCode(string binary_filename,
  ...
  // Flip the machine code bits
  int half_word = 8;
  vector<string> machine_code_flipped;
  for (int i = 0; i < machine_code_lines_.size(); ++i) {
    string code_line = machine_code_lines_.at(i);
    code_line = code_line.substr(half_word, half_word) + code_line.substr(0, half_word);
    machine_code_flipped.push_back(code_line);
  }
  // Convert the machine_code_lines_ vector to short integers
  vector<int16_t> binary_input;
  for (auto iter = machine_code_flipped.begin();
            iter != machine_code_flipped.end(); ++iter) {
